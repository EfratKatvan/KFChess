...
def test_build_board_view_state_reports_board_dimensions_and_game_over():
    ...
    assert view_state.scores == {}


def test_build_board_view_state_passes_through_the_scores_the_caller_supplies():
    """RealTimeArbiter no longer holds a running score itself (see
    ScoreObserver) - build_board_view_state just forwards whatever the
    caller already accumulated, the same pass-through already used for
    move_log."""
    board = Board(width=1, height=1)
    arbiter = RealTimeArbiter(board)

    view_state = build_board_view_state(board, arbiter, game_over=False, total_elapsed_ms=0, scores={WHITE: 9, BLACK: 3})

    assert view_state.scores == {WHITE: 9, BLACK: 3}
